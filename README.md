...
- 输出结构化规格（Pydantic）
- 自动生成设计方案（分步骤）
- 自动生成“专业重规划蓝图”（架构分层、里程碑、风险与人性化策略）
- 自动生成推荐项目骨架与用途说明
- 自动生成可直接复制的初始化文件（`README.md`、`pyproject.toml`、`src/main.py`、`.gitignore`）
- 支持“一键写入本地项目目录”，生成到 `generated_projects/<project>-<timestamp>/`
- 自动生成 Mermaid 逻辑流程图
- 界面支持科技风样式（深色渐变 + 信息卡片），提升专业感与可读性
- 自动生成核心算法逻辑拆解、API 职责地图与时序图
- 自动生成可行性体检评分与分阶段行动清单
- 自动生成用户视角描述与智能追问问题（防止需求遗漏）
- 自动生成创新点总结
- 通过 OpenAI / Qwen 生成增强方案文本与协作建议（需配置 API Key）
- GitHub 同步先做“专业预检”（Owner/Repo/分支命名校验 + 目标仓库确认），再生成可推送到指定仓库的脚本
